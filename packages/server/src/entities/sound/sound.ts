import { JSONSchema } from "vscode-json-languageservice";
import { SchemaConfiguration } from "vscode-json-languageservice";
import { Entity } from "../entity";
import { PointerType } from "../../pointers";
import { SchemaBuilder as _ } from "../../schema-builder";

export class Sound extends Entity {
    public config: SchemaConfiguration = { fileMatch: ["*.sound"], uri: "" };
    public path: string[] = [this.schemaManager.schemasPath_dev, "sound-schema.json"];

    public patch(): SchemaConfiguration {
        this.setUri(...this.path);
        const schema: JSONSchema = this.schemaManager.parseSchema(...this.path);

        const defs: any = schema.$defs;
        defs.ogg_ptr = _.string(PointerType.ogg);

        this.setSchema(schema);
        return this.config;
    }
}
