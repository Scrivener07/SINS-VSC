...
export interface EntityManifestType {
    weapon: Set<string>;
...
export class EntityManifestManager extends CacheStorage<EntityManifestType> {
    public async loadEntityManifestIds(rootPath: string, entityManifest: keyof EntityManifestType): Promise<void> {
    ...
    public async loadWeapons(rootPath: string): Promise<void> {
        await this.loadEntityManifestIds(rootPath, "weapon");
    }

    public async loadCache(rootPath: string): Promise<void> {
        await Promise.all([this.loadUnits(rootPath), this.loadUnitSkins(rootPath), this.loadUnitItems(rootPath), this.loadWeapons(rootPath)]);
    }
}
